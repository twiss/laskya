...
/* NaN and Infinity support copyright (c) 2009 Jon-Carlos Rivera */
...
//-- private static final java.lang.String $0="BigDecimal.nrx";
 ...
 BigDecimal.prototype.sqroot = sqroot;
 BigDecimal.prototype.log10 = log10;
 BigDecimal.prototype.log2 = log2;
 BigDecimal.prototype.ln = ln;
 BigDecimal.prototype.expn = expn;
 BigDecimal.prototype.pow = pow;
 BigDecimal.prototype.pow2 = pow2;
 BigDecimal.prototype.pi = pi;
 BigDecimal.prototype.e = e;
 ...
 BigDecimal.prototype.isnan = NaN;
 BigDecimal.prototype.ispinf = Infinity;
 BigDecimal.prototype.isninf = -Infinity;
...
BigDecimal.prototype.E = new BigDecimal("2.718281828459045235360287471352662497757247093699959574966967627724076630353547594571382178525166427427466391932003");
BigDecimal.prototype.PI = new BigDecimal("3.141592653589793238462643383279502884197169399375105820974944592307816406286208998628034825342117067982148086513282");
BigDecimal.prototype.LN_LOG10 = new BigDecimal("0.434294481903251827651128918916605082294397005803666566114453783165864649208870774729224949338431748318706106744766");
BigDecimal.prototype.LOG2_LOG10 = new BigDecimal("0.301029995663981195213738894724493026768189881462108541310427461127108189274424509486927252118186172040684477191430");
 
BigDecimal.prototype.NAN = new BigDecimal("NaN");
BigDecimal.prototype.PINF = new BigDecimal("Infinity");
BigDecimal.prototype.NINF = new BigDecimal("-Infinity");
 /**
...
BigDecimal.prototype.ONE = new BigDecimal("1");
 ...
 function BigDecimal() {
  ...
  if (length<=0) 
   return this.bad("BigDecimal(): ", inchars); // bad conversion (empty string)
  ...
  if (inchars[0]==('-')) 
   {
    ...
    if(inchars[1]==('I')){
	  	this.mant = this.iszero;
	    this.ind=this.isninf;
	    return;
	  }
    else
	    this.ind=this.isneg;
  ...
  }else if (inchars[0]==('+')) 
	{
	  ...
	  if(inchars[1]==('I')){
	  	this.mant = this.iszero;
	    this.ind=this.ispinf;
	    return;
	  }
	   offset++;
  }else if(inchars[0]==('I')||inchars[0]==('i')){
	   length--;
	   if (length==0) 
	    this.bad("BigDecimal(): ", inchars); // nothing after I
  	this.mant = this.iszero;
    this.ind=this.ispinf;
    return;
	}else if(inchars[0]==('N')||inchars[0]==('n')){
	   length--;
	   if (length==0) 
	    this.bad("BigDecimal(): ", inchars); // nothing after N
  	this.mant = this.iszero; 
    this.ind=this.isnan;
    return;
	}
...
MathContext.prototype.BETTER = new MathContext(31,MathContext.prototype.DEFAULT_FORM,MathContext.prototype.DEFAULT_LOSTDIGITS,MathContext.prototype.ROUND_UNNECESSARY);
 /**
...
function log2() {
	var set; 
  if (log2.arguments.length == 1)
   {
    set = log2.arguments[0];
   }
  else
   {
    set = this.plainMC;
  ...
  if(isNaN(this.ind))
   	return this.NAN;
	// Infinity Handling
  if(this.ind == this.ispinf)
   	return this.PINF;
  if(this.ind == this.isninf)
   	return this.NINF;
  // Make sure this is a positive number
  if (this.compareTo(this.ZERO) <= 0) {
  	return this.NAN; 
  }  else if(this.compareTo(this.ONE) == 0){
  	return this.ZERO; 
  }

   // TODO: Find a better way to calculate log2()
	 return(this.log10(set).divide(this.LOG2_LOG10, set).finish(set));
}
 
/*
//Power series natural log. sucks.

function ln2() {
	var set; 
  if (ln2.arguments.length == 1)
   {
    set = ln2.arguments[0];
   }
  else
  ...
  var mc = new MathContext(100,MathContext.prototype.DEFAULT_FORM,MathContext.prototype.DEFAULT_LOSTDIGITS,MathContext.prototype.ROUND_HALF_UP);

	var neg = false;
  var aux = this.subtract(this.ONE, set);

  if(this.compareTo(this.ONE) == 1) { // Greater than 1?
  	neg = true;
		aux = this.ONE.divide(this, mc).subtract(this.ONE, mc);
  }
  ...
  while (i < 600)
  {
  	bigi = bigi.add(this.ONE, mc);
  	power = power.multiply(aux, mc);
    result = result.subtract( power.divide(bigi, mc), mc);
  	bigi = bigi.add(this.ONE, mc);
  	power = power.multiply(aux, mc);
    result = result.add( power.divide(bigi, mc), mc);
    i+=2;
  }
 	bigi = bigi.add(this.ONE, mc);
 	power = power.multiply(aux, mc);
...
function ln() {
  ...
  if (this.compareTo(this.ZERO) <= 0) {
   ...
   // TODO: Find a better way to calculate ln()
	 return(this.log10(set).divide(this.LN_LOG10, set).finish(set));
}

function expn()
{
	var set; 
  if (expn.arguments.length == 1)
   {
    set = expn.arguments[0];
   }
  else
   {
    set = this.plainMC;
  ...
  var iteration = 0; 
  var result = this.ONE;
  var factorial = this.ONE;
  var	bigi = this.ONE;
  var power = this;
  while (iteration < 101) // Adjust to modify the precision - probably overkill
  {
    factorial = factorial.multiply(bigi, set);
    result = result.add(power.divide(factorial, set), set);
    power = power.multiply(this, set);
  	bigi = bigi.add(this.ONE, set);
    iteration++;
  }
  return(result.finish(set));
}
...
function log10() {
	var set; 
  if (log10.arguments.length == 1)
   {
    set = log10.arguments[0];
   }
  else
   {
    set = this.plainMC;
  ...
  if (this.compareTo(this.ZERO) <= 0) {
  	...
  	return this.ZERO; 
 				...
 				var b = this;
        var NUM_OF_DIGITS = set.digits+2; // need to add one to get the right number of dp
                                        //  and then add one again to get the next number
                                        //  so I can round it correctly.

        var mc = new MathContext(NUM_OF_DIGITS,MathContext.prototype.DEFAULT_FORM,MathContext.prototype.DEFAULT_LOSTDIGITS,MathContext.prototype.ROUND_HALF_UP);

        //special conditions:
        // log of a number lessthan one = -log(1/x)
        if(b.compareTo(this.ONE) < 0)
                return( this.ONE.divide(b).log10(set).negate() );

        //number of digits on the left of the decimal point
        var integer_part = new BigDecimal(b.format(-1,0,-1,-1,MathContext.prototype.SCIENTIFIC,MathContext.prototype.ROUND_DOWN));
        var leftDigits = integer_part.toString().length - (integer_part.toString().indexOf(".") > 0 ? integer_part.toString().indexOf(".") : 0);
        ...
        {
      		      b = (b.movePointLeft(leftDigits - 1)).pow(this.ONE.movePointRight(1), mc);
	        			var integer_part = new BigDecimal(b.format(-1,0,-1,-1,MathContext.prototype.SCIENTIFIC,MathContext.prototype.ROUND_DOWN));
	        			var leftDigits = integer_part.toString().length;
	        			var index_of_decimal = integer_part.toString().indexOf(".");
	        			if(index_of_decimal >= 0){
  		      			leftDigits = index_of_decimal;
	        			}
                sb = sb + "" + (leftDigits - 1);
                n++;
        };

        var ans = new BigDecimal(sb);

        //Round the number to the correct number of decimal places.
   //     ans = ans.round(new MathContext(ans.precision() - ans.scale() + dp, RoundingMode.HALF_EVEN));
        return ans.finish(set);
...
function sqroot() {
  var set; 
  if (sqroot.arguments.length == 1)
   {
    set = sqroot.arguments[0];
   }
  else
   {
    set = this.plainMC;
  ...
  if (this.compareTo(this.ZERO) <= 0) {
	...
	var initialApproximation = function(n) {
		var integerPart = new BigDecimal(n.format(-1,0,-1,-1,MathContext.prototype.SCIENTIFIC,MathContext.prototype.ROUND_DOWN));
		var length = integerPart.toString().length;
		if ((length % 2) == 0) length--;
		length = length >> 1;
		return this.ONE.movePointRight(length); 
	};
  ...
  //----------------
    ...
    var lastGuess;
    var guess = initialApproximation.call(this, n);
		var error = new BigDecimal("11111.0");

    // Iterate
		var maxIterations = 99; // Too many? Too few?
    var iterations = 0;
    var more = true;
    while(more) {
      lastGuess = guess;
      guess = n.dodivide('D',guess,set,31);
      guess = guess.add(lastGuess, set);
      guess = guess.dodivide('D',TWO,set,31);
      error = n.subtract(guess.multiply(guess, set)); 
      if (++iterations >= maxIterations) {
        more = false;
      }
      else if (lastGuess.equals(guess)) {
        more = error.abs().compareTo(this.ONE.movePointLeft(5)) >= 0;
      }
    }
    return guess.finish(set);
  }
 ...
 function add() {
  ...
  lhs=this; // name for clarity and proxy
 	...
 	// Nan poisoning
  if(isNaN(lhs.ind) || isNaN(rhs.ind))
   	return this.NAN;
	// Infinity Handling
  if(rhs.ind == this.ispinf)
   	return this.PINF;
  if(rhs.ind == this.isninf)
   	return this.NINF;
  if(lhs.ind == this.ispinf)
 		return this.PINF;
  if(lhs.ind == this.isninf)
 		return this.NINF;
 ...
 function negate() {
  ...
  if(isNaN(this.ind))
   	return this.NAN;
	// Infinity Handling
  if(set.ind == this.ispinf)
   	return this.NINF;
  if(set.ind == this.isninf)
   	return this.PINF;
 ...
 function pow2() {
  var set;
  if (pow2.arguments.length == 2)
   {
    set = pow2.arguments[1];
   }
  else if (pow2.arguments.length == 1)
   {
    set = this.plainMC;
   }
  else
   {
    throw "pow(): " + pow2.arguments.length + " arguments given; expected 1 or 2"
   }
  var rhs = pow2.arguments[0];
  var mc = new MathContext(set.digits+2,MathContext.prototype.DEFAULT_FORM,MathContext.prototype.DEFAULT_LOSTDIGITS,MathContext.prototype.ROUND_HALF_UP);

/* HACK to get non-integer powers */
	var n = new BigDecimal(rhs.format(-1,0,-1,-1,MathContext.prototype.SCIENTIFIC,MathContext.prototype.ROUND_DOWN));
	var decimal_part = rhs.subtract(n, set);
	
	if(decimal_part.compareTo(this.ZERO) > 0)
		var multiplier = decimal_part.multiply(this.ln(mc), mc).expn(mc);
	else
		var multiplier = this.ONE;
	
  return(multiplier.multiply(this.pow(n, mc), mc).finish(set));
}
 ...
 function layout() {
  ...
  if(isNaN(this.ind)){
  	return "NaN".split("");
  }else if(this.ind == this.ispinf){
  	return "Infinity".split("");
  }else if(this.ind == this.isninf){
  	return "-Infinity".split("");
  }
 ...
 function dodivide(code, rhs, set, scale) {
  ...
  if(isNaN(lhs.ind) || isNaN(rhs.ind))
   	return this.NAN;
	// Infinity Handling
  if(rhs.ind == this.ispinf || lhs.ind == this.ispinf)
   	return this.PINF;
  if(rhs.ind == this.isninf || lhs.ind == this.isninf)
   	return this.NINF;
  	
  // [note we must have checked lostDigits before the following checks]
  if (rhs.ind==0) {
  	if(lhs.ind == this.ispinf || lhs.ind == this.isninf)
  	  return this.NAN;
    if (lhs.ind == this.isneg) 
	   	return this.NINF;
	   else
	   	return this.PINF;
 ...
 function byteaddsub(a, avlen, b, bvlen, m, reuse) {
  ...
  //--int bp;
  	bp = bvlen - 1,
  //--int maxarr;
    maxarr = Math.max(ap, bp),
  //--byte reb[];
  	reb = reuse && maxarr + 1 == alength ? a : this.createArrayWithZeros(maxarr + 1),
  //--boolean quickm;
  	quickm = m == 1 || m == -1,
  //--int digit;
  	digit = 0,
  ...
  //--int dp90=0;
  	dp90 = 0,
  //--byte newarr[];
  	newarr = reuse && maxarr + 2 == a.length ? a : [],
  //--int i=0;
  	i = 0;
	...
	/*op: */for(;op>=0;op--) {
		...
		if(bp >= 0) {
			if (bp<blength) { // within B
				if(quickm) {
					if(m>0) digit = digit+b[bp]; // most common
					else digit = digit-b[bp]; // also common
				}
				else digit = digit+(b[bp]*m);
			}
			bp--;
		}
		/* result so far (digit) could be -90 through 99 *
		if (digit < 10) if (digit>=0) {
			quick: do { // 0-9
				reb[op] = digit;
				digit = 0; // no carry
				continue op;
	...
	if (digit==0) return reb; // no carry
